...
func TestSignVerifyState(t *testing.T) {
	...
	token := SignState("acme.myshopify.com", "gifting", "nonce123", "secret", now.Add(10*time.Minute))

	shop, brand, ok := VerifyState(token, "secret", now)
	if !ok || shop != "acme.myshopify.com" || brand != "gifting" {
		t.Errorf("verify = (%q, %q, %v), want acme.myshopify.com/gifting/true", shop, brand, ok)
	}
